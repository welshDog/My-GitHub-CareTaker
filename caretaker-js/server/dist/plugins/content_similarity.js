...
export async function contentSimilarity(owner, repoA, repoB, threshold = 0.85, redis) {
    const cacheKey = `content-sim:${owner}:${repoA}:${repoB}`;
    const cached = await redis.get(cacheKey);
    if (cached)
        return JSON.parse(cached);
    async function sample(repo) {
        ...
        const files = (tree.tree || []).filter((n) => n.type === 'blob' && /\.(ts|js|py|java)$/i.test(n.path));
        const stratified = files.filter((_, i) => i % Math.ceil(files.length / 20) === 0);
        const samples = [];
        for (const f of stratified) {
            ...
            if (cj && cj.content) {
                samples.push(Buffer.from(cj.content, 'base64').toString('utf-8'));
            }
        }
        return samples.join('\n');
    }
