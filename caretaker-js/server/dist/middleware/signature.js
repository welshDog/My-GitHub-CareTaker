...
export function rawBodyCapture() {
    return (req, _res, buf) => { req.rawBody = buf; };
}
export function verifySignature(redis) {
    return async (req, res, next) => {
        try {
            ...
            if (exists) {
                logger.error({ ts: Date.now(), ip: req.ip }, 'replay detected');
...
export async function rotateSecret(redis, newSecret) {
    await redis.lpush('webhook:secrets', newSecret);
    const len = await redis.llen('webhook:secrets');
    if (len > 12) {
        await redis.ltrim('webhook:secrets', 0, 11);
    }
