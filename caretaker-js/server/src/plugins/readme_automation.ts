...
export async function enhanceReadme(gh:GitHub, owner:string, repo:string, headerMd:string, svg:string, redis:Redis){
  ...
  const repoJson:any = await rr.json()
...
export async function rollbackReadme(owner:string, repo:string, redis:Redis){
