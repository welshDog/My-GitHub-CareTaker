...
type Repo = { name:string; description:string; stargazerCount:number; forkCount:number; updatedAt:string; pushedAt:string }

export async function recommendPins(gh:GitHub, username:string, redis:Redis){
  const cacheKey = `pins:${username}`
  const cached = await redis.get(cacheKey)
  if(cached) return JSON.parse(cached)
  const repos:Repo[] = await gh.reposByUser(username)
  const now = Date.now()
