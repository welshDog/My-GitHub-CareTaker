...
type PushResult = { ok:boolean, logPath:string }

export async function ensureBranchProtection(gh:GitHub, owner:string, repo:string){
  ...
  if(!res.ok) logger.warn({ status: res.status }, 'branch protection update failed')
}

async function runChecks(cwd:string){
  const log:any = {}
  if(fs.existsSync(path.join(cwd,'package.json'))){
    try{ await execa('npm',['ci'],{ cwd }) ; log.npm_ci='ok' } catch(e:any){ log.npm_ci=e.message; throw e }
    try{ await execa('npm',['test','--silent'],{ cwd }) ; log.tests='ok' } catch(e:any){ log.tests=e.message; throw e }
    try{ await execa('npm',['run','build','--silent'],{ cwd }) ; log.build='ok' } catch(e:any){ log.build=e.message; throw e }
    try{ const r = await execa('npm',['audit','--json'],{ cwd }) ; log.audit=JSON.parse(r.stdout) } catch(e:any){ log.audit=e.stdout || e.message }
  }
  return log
}

export async function securePushFlow(repoUrl:string, branch='main'): Promise<PushResult>{
  ...
  const auditDir = path.join(process.cwd(),'logs'); fs.mkdirSync(auditDir,{ recursive:true })
...
export async function rollback(repoUrl:string, tag:string){
