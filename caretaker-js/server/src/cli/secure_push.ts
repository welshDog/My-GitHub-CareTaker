...
rl.question(`Proceed with secure push to ${repo}? Type YES to continue: `, async (ans)=>{
  if(ans !== 'YES'){ console.log('Aborted'); rl.close(); return }
  try{ const r = await securePushFlow(repo); console.log('OK', r.logPath) }
  catch(e){ console.error('Failed', e); await rollback(repo, 'backup/main'); }
  rl.close()
})
